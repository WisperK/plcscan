...
def scan(argv):
    ...
    if options.hosts_file:
        try:
            with open(options.hosts_file, 'r', encoding='utf-8') as hosts_file:
                scan_hosts = [line.strip() for line in hosts_file if line.strip()]
        except IOError:
    ...
    for host in scan_hosts:
        splitted = host.split(':')
        host = splitted[0]
        ports = [int(splitted[1])] if len(splitted) == 2 else scan_ports
